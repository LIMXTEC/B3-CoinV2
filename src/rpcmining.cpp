...
Value getsubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfWorkReward(0, pIndex0->nHeight+1);
}
...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest, nCoinAge, 0, pIndex0->nHeight+1);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkReward(0, pIndex0->nHeight+1)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "B3-Coin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "B3-Coin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "B3-Coin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "B3-Coin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if(pblock->payee != CScript()){
        ...
        result.push_back(Pair("payee_amount", (int64_t)GetFundamentalnodePayment(pindexPrev->nHeight+1, pblock->vtx[0].GetValueOut())));
    } else {
    ...
    if(TestNet()){
        if(pblock->nTime > START_FUNDAMENTALNODE_PAYMENTS_TESTNET) FundamentalnodePayments = true;
    } else {
        if(pblock->nTime > START_FUNDAMENTALNODE_PAYMENTS) FundamentalnodePayments = true;
    }


    result.push_back(Pair("fundamentalnode_payments", FundamentalnodePayments));
